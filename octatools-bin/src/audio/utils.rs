...
use crate::RBoxErr;
use octatools_lib::constants::OCTATRACK_COMPATIBLE_AUDIO_SPECS;
...
fn file_path_is_compat_wav(path: &PathBuf) -> RBoxErr<bool> {
    ...
    trace!("Creating octatools_lib AudioSpec: path={path:#?}");
    let audio_spec = octatools_lib::constants::AudioSpec {
