...
fn walkdir_filter_is_compat_wav(entry: &DirEntry) -> bool {
    ...
    if wav_reader_res.is_err() {
        error!(
            "Could not read WAV file: wavfile={:#?} error={:#?}",
            &wav_path,
            wav_reader_res.err().unwrap().to_string()
        );
        return false;
    }
    ...
    if wav_spec_res.is_err() {
        error!(
            "Could not read WAV file WavSpec: wavfile={:#?} error={:#?}",
            &wav_path,
            wav_spec_res.err().unwrap().to_string()
        );
        return false;
    }
    ...
    let paths_iter: _ = WalkDir::new(dir_path)
        ...
        .filter_entry(|e| {
            ...
            walkdir_filter_is_not_hidden(e) && {
                ...
                walkdir_filter_is_dir(e) || walkdir_filter_is_compat_wav(e)
            }
        });
