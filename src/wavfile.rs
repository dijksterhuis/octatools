...
*/
use std::{error::Error, path::PathBuf, io::BufReader, fs::File};
...
pub fn chain_wavfiles_64_batch(wavfiles: &Vec<WavFile>) -> Result<Vec<WavFile>, ()> {

    let originals: Vec<WavFile> = wavfiles.clone();
    let mut slice_vecs: Vec<Vec<WavFile>> = vec![];

    let vec_mod_length = wavfiles.len() % 64;

    for i in 0..(vec_mod_length + 1) {
        let mut s: Vec<WavFile> = vec![];
        s.clone_from_slice(&originals[(i * 64)..(i * 64)]);
        slice_vecs.push(s);
    };

    let mut chains : Vec<WavFile> = vec![];
...
pub struct WavFile {
    pub spec: WavSpec,
    pub len: u32,
    pub samples: Vec<i16>,  // cannot use Copy trait
}
...
impl WavFile {
    ...
    pub fn from_file(wav_file_path: &PathBuf) -> Result<WavFile, Box<dyn Error>> {
        let mut reader = WavFile::reader(&wav_file_path).unwrap();
    ...
    fn reader(path: &PathBuf) -> Result<WavReader<BufReader<File>>, hound::Error> {
    ...
    /// Write samples to a WAV file.
    fn read_wav_samples(reader: &mut WavReader<BufReader<File>>) -> Result<Vec<i16>, Box<dyn Error>> {

        let samples_iter = reader.samples::<i16>();
...
fn walkdir_filter_is_compat_wav(entry: &DirEntry) -> bool {
    ...
    let mut wav_reader = WavFile::reader(&wav_path).unwrap();
    let wav_spec = WavFile::read_wav_spec(&mut wav_reader).unwrap();

    OCTATRACK_COMPATIBLE_HOUND_WAVSPECS.contains(&wav_spec.clone())
